...
export async function sendTweet(
    ...
    const result = mediaBuffer
        ? await client.twitterClient.sendTweetWithMedia(processedChunk, [mediaBuffer], inReplyTo)
        : await client.requestQueue.add(() => client.twitterClient.sendTweet(processedChunk, inReplyTo));
    // console.log("send tweet result:\n", result);
    const body = await result.json();
    console.log("send tweet body:\n", body.data.create_tweet.tweet_results);
    const tweetResult = body.data.create_tweet.tweet_results.result;
