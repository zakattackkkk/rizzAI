...
export class ApprovalQueue {
    ...
    async add(content: string, metadata: any = {}): Promise<string> {
        ...
        await run(
            'INSERT INTO pending_tweets (id, content, metadata) VALUES (?, ?, ?)',
    ...
    async approve(id: string): Promise<void> {
        ...
        const result = await run(
            'UPDATE pending_tweets SET status = ? WHERE id = ? AND status = ?',
            ['approved', id, 'pending']
        );
        if (result.changes === 0) {
            throw new Error(`Tweet ${id} not found or not in pending status`);
        }
    }
