...
export function isPlaceBuildingContent(
    content: PlaceBuildingContent
): content is PlaceBuildingContent {
    // Validate types
    ...
    const validBuildingCategory = Object.values(BuildingCategory)
...
export const placeBuilding: Action = {
    ...
    handler: async (
        ...
        try {
            const client = new GraphQLClient(process.env.STARKNET_INDEXER_URL);
            const resources = await client.request<ResourceResult>(
                resourceQuery,
                {
                    entityId: 62,
                }
            );
            const positions = await client.request<PositionResult>(
                positionQuery,
                {
                    entityId: 62,
                }
            );
            const buildings = await client.request<BuildingResult>(
                buildingQuery,
                {
            ...
            const buildingsParsed = buildings.eternumBuildingModels.edges.map(
                (edge) => ({
            ...
            const resourcesParsed =
                resources.eternumResourceModels.edges.reduce(
                    (acc, edge) => ({
                        ...acc,
                        [Number(edge.node.resource_type)]:
                            Number(edge.node.balance) /
                            EternumGlobalConfig.resources.resourcePrecision,
                    }),
                    {}
                );

            const neighboringHexes = getNeighborHexes(
                REALM_CENTER_POSITION.col,
                REALM_CENTER_POSITION.row
            );

            const emptyNeighborHexes = neighboringHexes.filter(
                (hex) =>
                    !buildingsParsed.some(
                        (building) =>
                            building.inner_col === hex.col &&
                            building.inner_row === hex.row
                    )
            );

            const costs =
                EternumGlobalConfig.buildings.buildingCosts[
                    response.buildingCategory
                ];
            ...
            const buildingContract = new Contract(
                buildingAbi,
                buildingContractAddress,
                provider
            );
            ...
            const direction = getDirectionBetweenAdjacentHexes(
                REALM_CENTER_POSITION,
                emptyNeighborHexes[0]
            );

            const buildingEnum = new CairoCustomEnum({
                None:
                    response.buildingCategory === BuildingType.None
                        ? 0
                        : undefined,
                Castle:
                    response.buildingCategory === BuildingType.Castle
                        ? 1
                        : undefined,
                Resource:
                    response.buildingCategory === BuildingType.Resource
                        ? 2
                        : undefined,
                Farm:
                    response.buildingCategory === BuildingType.Farm
                        ? 3
                        : undefined,
                FishingVillage:
                    response.buildingCategory === BuildingType.FishingVillage
                        ? 4
                        : undefined,
                Barracks:
                    response.buildingCategory === BuildingType.Barracks
                        ? 5
                        : undefined,
                Market:
                    response.buildingCategory === BuildingType.Market
                        ? 6
                        : undefined,
                ArcheryRange:
                    response.buildingCategory === BuildingType.ArcheryRange
                        ? 7
                        : undefined,
                Stable:
                    response.buildingCategory === BuildingType.Stable
                        ? 8
                        : undefined,
                TradingPost:
                    response.buildingCategory === BuildingType.TradingPost
                        ? 9
                        : undefined,
                WorkersHut:
                    response.buildingCategory === BuildingType.WorkersHut
                        ? 10
                        : undefined,
                WatchTower:
                    response.buildingCategory === BuildingType.WatchTower
                        ? 11
                        : undefined,
                Walls:
                    response.buildingCategory === BuildingType.Walls
                        ? 12
                        : undefined,
                Storehouse:
                    response.buildingCategory === BuildingType.Storehouse
                        ? 13
                        : undefined,
                Bank:
                    response.buildingCategory === BuildingType.Bank
                        ? 14
                        : undefined,
                FragmentMine:
                    response.buildingCategory === BuildingType.FragmentMine
                        ? 15
                        : undefined,
            });
            ...
            const call = await buildingContract.invoke("create", [
                62,
                [directionEnum],
                buildingEnum,
                none,
...
const buildingQuery = gql`
    query BuildingQuery($outer_col: Int!, $outer_row: Int!) {
        eternumBuildingModels(
            where: { outer_col: $outer_col, outer_row: $outer_row }
        ) {
            edges {
                node {
                    inner_col
